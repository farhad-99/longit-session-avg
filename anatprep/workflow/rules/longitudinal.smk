...
Longitudinal within-subject workflow rules:
  1. all_within_subject   — terminal rule (default target)
  2. trim_and_center      — clip intensities, re-centre origin
  3. build_subject_template — build unbiased average across sessions
  4. register_sessions_to_template — rigidly register each session to template
"""


# ── terminal rule (must be first so Snakemake picks it as default) ────────────
